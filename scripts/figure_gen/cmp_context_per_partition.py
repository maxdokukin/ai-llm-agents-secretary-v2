...
def plot_grouped_metrics_template(df, out_path="avg_utilization_by_metric_model.png"):
    """
    Processes the DataFrame and plots the grouped differences using the new tags.
    """
    if df.empty:
    ...
    # 2. Define the new metrics to plot
    metrics = [
        "master",
        "tool_definitions",
        "tool_results",
    ...
    # 4. Plot grouped bars
    fig, ax = plt.subplots(figsize=(11, 6))  # Widened slightly for longer labels
    ...
    # Formatting
    ax.set_title("Average Context Utilization by Metric and Model", fontsize=14, fontweight="bold")
    ax.set_xlabel("Utilization Metric", fontsize=12)
    ax.set_ylabel("Average Tokens / Count", fontsize=12)
    ax.set_xticks(x)
    # Added rotation and ha="right" so the new longer names look clean
