...
# Import the newly rewritten Context Manager
...
cm = ContextManager(max_context_size=8192)
...
def set_context_size(payload: SizePayload):
    ...
    return {"status": "success", "max_context_size": cm.max_context_size}


@app.get("/api/context/size")
def get_context_size():
    return {"max_context_size": cm.max_context_size}
...
def get_nicer_bar():
    ...
    <body>
        ...
        <p>Total Context Window Limit: <b>{max_size}</b> chars</p>
        ...
        <div class="legend">
            <div><span class="dot" style="background-color: #cba6f7;"></span>Used Space ({used}c)</div>
            <div><span class="dot" style="background-color: #313244;"></span>Free Space ({free}c)</div>
        </div>

        <h3 style="margin-top: 30px;">Raw Compilation Preview</h3>
        <pre style="background: #11111b; padding: 15px; border-radius: 8px; overflow-x: auto; font-size: 12px;">{__import__('json').dumps(cm.get_context(), indent=2)}</pre>
    </body>
    </html>
    """
    return html_content
